...
CV<std::size_t, decltype(speed_range)> speed {speed_range};
...
CV<double, decltype(func)> var {func};
...
CV<std::vector<int>, decltype(transformer_opt), decltype(not_empty_opt)> var {transformer_opt, not_empty_opt};
...
CV<bool, decltype(in_set_opt), decltype(transformer_opt)> var {in_set_opt, transformer_opt};
...
You can directly use `ValidBoolVariable` if you need a boolean variable validated against a set of constraints.
